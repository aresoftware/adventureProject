...
function WebRTCAdaptor(initialValues)
{
			...
			thiz.remotePeerConnection[streamId].oniceconnectionstatechange = function (event) {
				...
				if (!thiz.isPlayMode) {
					if (thiz.remotePeerConnection[streamId].iceConnectionState == "connected") {
						...
						thiz.constantResolution(streamId).then(() => {
                             console.log("constantResolution is activated");
                         })
                        .catch(e => console.error(e));
					}
				}
    ...
    this.constantResolution = function(streamId) {
    		...
    		if (videoSender != null) {
    			...
    			errorDefinition = "Video sender not found to change degradationPreference";
    		}
